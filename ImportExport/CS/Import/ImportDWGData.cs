...
namespace Revit.SDK.Samples.ImportExport.CS
{
    ...
    ///// 
    ///// </summary>
    //public class MyPropertyLine : Element
    //{
    ...
    public class ImportDWGData : ImportData
    {
        ...
        public override bool Import()
        {
            ...
            #region set layer(catogery)
            List<string> layers = dwgImportOption.GetLayerSelection().ToList();
            ICollection<string> sels = new Collection<string>();
            sels.Add("C01出让(划拨)用地红线@许新");
            //sels.Add("08S1城市道路");
            //sels.Add("08S1城市道路2");
            dwgImportOption.SetLayerSelection(sels);
            layers = dwgImportOption.GetLayerSelection().ToList();
            #endregion
            ...
            ImportInstance elem = m_activeDoc.GetElement(elementId) as ImportInstance;
            ...
            var it = subcat.GetEnumerator();
            it.Reset();

            List<ElementId> dels = new List<ElementId>();
            while (it.MoveNext())
            {
                Category tmp = it.Current as Category;
                string name = tmp.Name;
                names.Add(name);

                if (name == "08S1城市道路")
                {
                    tmp.set_Visible(view, true);//隐藏图层
            ...
            //var goo = geo.GraphicsStyleId;

            void get(GeometryElement ge)
            {
                foreach(GeometryObject go in ge)
                {
                    
                    GraphicsStyle gs = m_activeDoc.GetElement(go.GraphicsStyleId) as GraphicsStyle;
                    string gobj_cat = gs?.GraphicsStyleCategory.Name;
                    if (go is Arc)
                    {
                        var goid = go.GraphicsStyleId;
                        array.Append(go as Arc);
                    }

                    if (go is Line)
                    {
                        array.Append(go as Line);
                    }

                    if (go is PolyLine)
                    {
                        var pl = go as PolyLine;
                        ...
                        //var end = Line.CreateBound(points[points.Count-1], points[0]);
                        //array.Append(go);
                    }

                    if (go is GeometryInstance)
                    {
                        var tmp = go as GeometryInstance;
                        var t2 = tmp.GetInstanceGeometry();

                        //get(tmp.SymbolGeometry);
                        get(t2);
                    }
                }
            }
            #region get sub elements
            //var subelems = elem.GetSubelements(); //empty!!
            ...
            //del
            //foreach (var id in dels)
            //{

            //    m_activeDoc.Delete(id);
            //}
            m_activeDoc.Delete(elementId);//删除导入的dwg
            CreateCurveLines(m_activeDoc, array);//创建模型线
            t.Commit();
            


            //Transaction tdel = new Transaction(m_activeDoc);
            //tdel.SetName("del");
            //tdel.Start();
            //foreach(var id in dels)
            //{
